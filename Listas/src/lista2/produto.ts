class ProdutoEncap{
    ...
    setQtde(qtde: number): void{
        if (qtde >= 0){
            ...
            console.log(`quantidade negativa não é permitida`)
        }
    }
    setPreco(preco: number): void{
        if (preco >= 0){
            ...
            console.log(`preço negativo não é permitida`)
        }
    }
    getQtde(): number {
        return this.qtde
...
let obj3ProdEncap = new ProdutoEncap(3, "calcular", 30, 1700)

console.log (`Qtde ${obj3ProdEncap.getQtde} e Preco: ${obj3ProdEncap.getPreco} ` )